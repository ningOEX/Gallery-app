<template>
	<view v-if="!isPublishedMoreThanSevenDaysAgo(image.timestamp)" class="card">
		...
		<view class="card-footer">
			...
			<text class="username">{{ image.nickname }}</text>
...
<script setup lang="ts">
	...
	import {isPublishedMoreThanSevenDaysAgo} from "@/utils/index"
	const props = defineProps<{image : ImagesForm}>()

	const toImageDetails = () => {
		uni.navigateTo({ url: `/pages/imageDetails/index?id=${props.image.uid}` })
	}
</script>
