...
class E {
  ...
  async getUserRecord() {
    ...
    if (this.oldTokenPayload) {
      if (this.isTokenInDb(this.oldTokenPayload.uniIdVersion)) {
        if (-1 === (this.userRecord.token || []).indexOf(this.oldToken))
          throw { errCode: n.CHECK_TOKEN_FAILED }
      }
      if (
        this.userRecord.valid_token_date &&
        this.userRecord.valid_token_date > 1e3 * this.oldTokenPayload.iat
      )
  ...
  async getUserPermission() {
    ...
    if (0 === e.length)
      return (this.userPermission = { role: [], permission: [] }), this.userPermission
    if (e.includes('admin'))
      return (this.userPermission = { role: e, permission: [] }), this.userPermission
    const t = await T.where({ role_id: I.in(e) }).get(),
      n =
        ((i = t.data.reduce((e, t) => (t.permission && e.push(...t.permission), e), [])),
  ...
  async createToken({ uid: e, role: t, permission: i } = {}) {
    ...
    this.uid = e
    const { token: r, tokenExpired: o } = await this._createToken({
      uid: e,
      role: t,
      permission: i,
    })
...
class x {
  constructor({ context: e, clientInfo: t, config: n } = {}) {
    ;(this._clientInfo = e
      ...
      w.hasFile('custom-token.js') &&
        this.setInterceptor('customToken', require(w.resolve('custom-token.js')))
